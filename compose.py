    ...
    for name, version in rpm_dict_f2.items():
        try:
            if rpm_dict_f1[name] != rpm_dict_f2[name]:
                print(
                    name + " CHANGED " + rpm_dict_f1[name] + " -> " + rpm_dict_f2[name]
                )
...
def main() -> None:
    ...
    parser.add_argument(
        "-u",
        "--url",
        default="https://kojipkgs.fedoraproject.org/compose/rawhide/",
        dest="url",
    ...
    parser.add_argument(
        "-r",
        "--rpmsjson",
        default="/compose/metadata/rpms.json",
        dest="rpmsjson",
    ...
    parser.add_argument(
        ...
        action="append",
        dest="two_composes",
        help='specify TWO composes: -d one -d two for.ex. --diff Fedora-Rawhide-20250807.n.1 --diff Fedora-Rawhide-20250814.n.0"',
    )
    ...
    if args.list_composes is not None:
        ...
        )  # add True for order = 1. line as newest compose
    ...
    if args.two_composes:
        if len(args.two_composes) == 2:
            if (
                args.two_composes[0] in wwwsubdirs_list
                ...
                #    "/tmp/tmpyqg3625o/Fedora-Rawhide-20250821.n.0-rpms.json",
                #]
